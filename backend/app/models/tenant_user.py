...
class TenantUser(db.Model):
    ...
    user_id = db.Column(db.Integer, nullable=False)  # 管理员ID
    ...
    # 定义关系（用于ORM查询，但无外键约束）
    # 使用foreign()注解明确标识外键列
    tenant = db.relationship('Tenant', primaryjoin='TenantUser.tenant_id == foreign(Tenant.id)', backref='admin_users')
    user = db.relationship('User', primaryjoin='TenantUser.user_id == foreign(User.id)', backref='tenants')
